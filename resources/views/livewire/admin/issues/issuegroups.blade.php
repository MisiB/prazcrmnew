<div>
    <x-card title="Issue groups" shadow separator>
        <x-slot:menu>
            <x-button icon="o-plus" @click="$wire.newdrawer = true" class="btn-circle btn-outline"/>
        </x-slot:menu>

        <x-table :headers="$headers" :rows="$groups" separator progress-indicator show-empty-text empty-text="Nothing Here!">

            @scope('actions', $menu)
            <div class="flex items-center justify-center">
                @can("modify.issuegroup")
    ...
    <x-drawer wire:model="newdrawer" title="New Issue group" right separator with-close-button class="lg:w-1/3">
    ...
    <x-drawer wire:model="editdrawer" title="Edit Issue group" right separator with-close-button class="lg:w-1/3">
