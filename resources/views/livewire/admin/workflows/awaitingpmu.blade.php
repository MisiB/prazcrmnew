<div>
    ...
    <x-modal wire:model="awardmodal" title="Award Details" separator box-class="max-w-3xl" progress-indicator>
        <x-form wire:submit.prevent="save">
            <x-card title="Customer" subtitle="Customer details" separator class="mt-5 border-2 border-gray-200">
                <x-slot:menu>
                   <x-input label="Regnumber" wire:model.live="regnumber"/>
                </x-slot:menu>
                @if($customer!=null)
                <table class="table table-xs table-zebra">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $customer->default_email }}</td>
                        </tr>
                    </tbody>
                </table>
                @else
                <x-alert class="alert-error" title="No data to display please enter regnumber to search"/>
                @endif
            </x-card>
            @if($customer!=null)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <x-slot:actions>
                <x-button label="Close" wire:click="$wire.awardmodal = false"/>
    ...
    <x-modal wire:model="documentmodal" title="Documents" separator box-class="max-w-3xl" progress-indicator>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="flex justify-end space-x-2">
                               ...
                               <x-button icon="o-trash" class="btn-outline btn-xs btn-error" wire:click="deletedocument({{ $award->id }})" wire:confirm="Are you sure?" />
        ...
        <x-slot:actions>
            <x-button icon="o-plus" class="btn-circle btn-primary" @click="$wire.awarddocumentmodal = true"/>
        </x-slot:actions>
    </x-modal>
    <x-modal wire:model="awarddocumentmodal" title="Award Document" separator progress-indicator>
        <x-form wire:submit.prevent="savedocument">
            <x-select wire:model="document" placeholder="Select Document Type" :options="$documentlist" option-label="name" option-value="id"/>
            <x-input type="file" wire:model="file"/>
            <x-slot:actions>
                <x-button label="Close" wire:click="$wire.awarddocumentmodal = false"/>
                <x-button label="Save" type="submit" class="btn-primary" spinner="savedocument"/>
            </x-slot:actions>
        </x-form>
    </x-modal>
