...
class Storesrequisitionverification extends Component
{
    Use Toast;
    ...
    public function mount($approvalrecordid, $approvalitemuuid)
    {
        ...
        $this->approvalrecord=$this->storesrequisitionService->getadminrequisitionapprovalrecord($this->approvalrecordid);
    ...
    { 
        ...
        $adminissuers=$this->storesrequisitionService->getadminissuersbyrole('Admin Issuer');
                
        $this->validate([
            'comment' => 'required'
        ]);
        $hodapprovalresponse=$this->storesrequisitionService->updateadminrequisitionrecord($this->storesrequisitionuuid,[
        ...
        if(!$this->isapproved) {
            $requisitionapprovalresponse=$this->storesrequisitionService->updateadminrequisitionrecord($this->storesrequisitionuuid,[
                'status'=>'R',
            ]);
            //send email notification to initiator
            $initiator->notify(new StoresrequisitiondeliveryNotification($this->storesrequisitionService, $this->storesrequisitionuuid) );
            ...
            $adminissuers->each(function($adminissuer) {
                ...
                $issuer->notify(new StoresrequisitiondeliveryNotification($this->storesrequisitionService, $this->storesrequisitionuuid) );
            });
            $initiator->notify(new StoresrequisitiondeliveryNotification($this->storesrequisitionService, $this->storesrequisitionuuid) );
    ...
    {
        return view('livewire.admin.workflows.approvals.storesrequisitionverification');
    }
}
